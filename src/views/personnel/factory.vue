    ...
    <div class="app-container">
        ...
        <div class="content">
            <div class="lgm-search">
                <el-form ref="form" :model="searchParams" label-width="80px" class="search-form">
                    <el-form-item label="加工厂">
                        <el-input v-model="searchParams.factory_name" placeholder="请输入加工厂名称查询"></el-input>
                    </el-form-item>
                    <el-row>
            ...
            <div class="table">
                <el-table :data="dataList" border>
                    <el-table-column prop="factory_name" label="加工厂名称" align="center"></el-table-column>
        ...
        <el-dialog title="岗位" :visible.sync="dialogVisible" center width="500px">
            <el-form :model="detailForm" :rules="rules" ref="ruleForm" label-width="100px" class="detailForm">
                <el-form-item label="加工厂名称" prop="factory_name" style="width: 100%;">
                    <el-input v-model="detailForm.factory_name"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
			...
			dialogConfirm() {
				...
				factorieAdmin(this.detailForm, this.detailForm.id ? 'PATCH' : 'POST',this.detailForm.id).then((e) => {
			...
			handleDelete(index, item) {
                this.$confirm('是否确认删除选中的加工厂?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					confirmButtonClass: 'submitDeleteBtn',
					type: 'warning'
				}).then(() => {
					factorieAdmin({}, 'DELETE', item.id).then((e) => {
						this.seach()
