  ...
  <div class="app-container">
      ...
      <div class="lgm-search">
        ...
        >
          <el-form-item label="款式名称">
            <el-input
              v-model="searchParams.style_name"
              placeholder="请输入款式编号或名称查询"
            ></el-input>
          </el-form-item>
          <el-row>
            <el-button
            ...
            <template slot-scope="scope">
              <div class="flex">
                ...
                <el-button
                  ...
                  type="success"
                  plain
                  @click="handleCcbf(scope.$index, scope.row)"
                  >裁床编菲</el-button
                >
    ...
    >
      ...
      >
              ...
              >
                ...
                <div class="header-right">
                  <el-button
                    type="primary"
                    class="lgm-add-btn"
                    @click="selectAllColor"
                    >{{ isAllSelectColorBtn }}</el-button
                  >
                </div>
              </div>
            </div>
            <el-option
              v-for="item of colorListOptions"
              :key="item.id"
              ...
              >
                ...
                <div class="header-right">
                  <el-button
                    ...
                    class="lgm-add-btn"
                    @click="selectAllSize"
                    >{{ isAllSelectSizeBtn }}</el-button
                  >
                </div>
              </div>
            </div>
            <el-option
              v-for="item of sizeListOptions"
              :key="item.id"
        ...
        <el-form-item label="款式分类" class="lgm-form-item">
          <el-select
            v-model="detailData.style_category_id"
            placeholder="请选择"
            style="width: 100%"
          >
            <div class="search-slot" style="padding: 10px">
            ...
            <el-option
              v-for="item of styleListOptions"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            >
        ...
        <el-form-item label="加工部门" class="lgm-form-item">
          <el-select
            v-model="detailData.department_id"
            placeholder="请选择部门"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item label="" class="lgm-form-item"> </el-form-item>
        ...
        <el-form-item
          ...
          style="width: 100% !important"
        >
          <el-button type="info" size="small" @click="toTemplate = true" plain
            >选择模板</el-button
          >
        </el-form-item>
        ...
        <el-form-item label="单价模式" style="width: 100% !important">
          <el-radio-group
            v-model="detailData.unit_price_mode"
            @input="modeChange"
              ...
              <template slot-scope="scope">
                ...
                >
                  <div
                    ...
                    @click="
                      () => {
                        scope.row.post_ids = adminroleList.map(
                          (role) => role.id
                        );
                      }
                    "
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="lgm-reset-btn" @click="dialog = false"
          >取消</el-button
        >
        <el-button type="primary" @click="styleAdminAdd" class="lgm-search-btn"
          >确定</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <div class="lgm-search">
        <el-form
          ref="form"
          :model="styleSearchParams"
          label-width="80px"
          class="search-form"
        >
          <el-form-item label="款式名称">
            <el-input
              v-model="styleSearchParams.style_name"
              placeholder="请输入款号或名称查询"
            ></el-input>
          </el-form-item>
          <el-row>
            <el-button
    ...
    >
      <div class="preview-info">
        <el-row :gutter="20" style="margin-bottom: 10px">
          ...
          <el-col :span="8">
            <div class="grid-content bg-purple">床次：{{ cdFormData.cs }}</div>
          </el-col>
          <el-col :span="8">
            <div class="grid-content bg-purple">总扎数：{{ totalZs }}</div>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="8">
            <div class="grid-content bg-purple">总裁：3</div>
          </el-col>
          <el-col :span="8">
            <div class="grid-content bg-purple">
      ...
      <el-table :data="diyTableData" ref="diyTable" border show-summary>
        ...
        <el-table-column
          align="center"
          :prop="'gm' + index"
          v-for="(item, index) of diyTableColumn"
          :key="item.label"
        >
          <template slot="header" slot-scope="scope">
      ...
      <div class="bottom-btn" style="margin: 20px 0; text-align: center">
        <el-button class="lgm-reset-btn" @click="previewDialog = false"
          >取消</el-button
        >
      </div>
    </el-dialog>
    ...
    <bedManage ref="bedManage" :isType="1" />
  </div>
</template>
...
import bedManage from "@/views/bed-manage/index.vue";
  ...
  methods: {
    ...
    saveAndPreview(cdForm) {
      this.$refs[cdForm].validate((valid) => {
        if (valid) {
          if (
            this.diyTableColumn.length === 0 ||
            this.diyTableData.length === 0
          ) {
            this.$message({
              message: "请先设置裁床表",
              type: "error",
    ...
    deleteTableColumn2(item, index, index2) {
      ...
      $("#diyTable .el-table__footer-wrapper .has-gutter tr")
        .find("td")
        .eq(index2 + 1)
        .find(".cell")
        .html(0);
    ...
    topDualChange(index, e) {
      for (let i in this.diyTableColumn) {
        ...
        for (let k in this.diyTableData) {
          ...
          if (Number(k) !== 0) {
            item2[label] =
              Number(this.diyTableData[0][label]) * Number(item2.dual);
          }
        }
      }
    },
    ...
    handleCcbf(idx, item) {
      this.$refs.bedManage.handleEdit(item);
    },
