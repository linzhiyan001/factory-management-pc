		...
		<div class="content">
			<div class="lgm-search">
				<el-form ref="form" :model="searchParams" label-width="80px" class="search-form">
					<el-form-item label="模版名称">
						<el-input v-model="searchParams.code2" placeholder="请输入模版名称查询"></el-input>
					</el-form-item>
					<el-row>
						<el-button @click="
                page = 1;
			...
			<div class="table">
				<el-table :data="dataList" border>
					...
					<el-table-column prop="unit_price_mode" label="单价模式" align="center">
		...
		<el-dialog title="工序" :visible.sync="dialogVisible" center width="1200px">
			<el-form :model="detailForm" :rules="rules" ref="ruleForm" label-width="100px" class="detailForm">
				...
				<el-form-item label="单价模式" style="width: 100%">
					<el-radio-group v-model="detailForm.unit_price_mode" :disabled="modeDisabled">
			...
			<div class="detail-table">
				<el-table :data="dataList2" border ref="dragTable" style="width: 100%">
					...
					<el-table-column prop="price" align="center" min-width="160">
						<template slot="header" slot-scope="scope">
							<el-popover placement="bottom" title=" " popper-class="table-popover" width="400"
								trigger="hover" v-if="detailForm.unit_price_mode === '2'">
								<span slot="reference">
                      ...
                      " :class="{
                        ...
                        isDisabled: item.disabled,
							...
							<el-popover placement="bottom" title=" " popper-class="table-popover" width="400"
								trigger="hover" v-else-if="detailForm.unit_price_mode === '3'">
								<span slot="reference">
									价格 <i class="el-icon-plus"></i>
								</span>
								<slot>
									<div class="size-list" style="display: flex; flex-wrap: wrap">
										<div class="size-list-item" v-for="(item, index) of gwList" :key="index" style="
                        padding: 8px 15px;
                        border: 1px solid #eee;
                        cursor: pointer;
                      " :class="{
						...
						<template slot-scope="scope">
							<el-input-number v-model="scope.row.price" controls-position="right" @input="totalPriceFn"
								:min="0" size="small" />
						</template>
					</el-table-column>
					...
					<el-table-column prop="price2" label="显示价格" align="center" min-width="80">
						<template slot-scope="scope">
							<el-switch v-model="scope.row.display_price" active-color="#ed6d00" />
						</template>
					</el-table-column>
					<el-table-column prop="post_ids" label="可见岗位" align="center" min-width="180">
						<template slot-scope="scope">
							<el-select v-model="scope.row.post_ids" multiple collapse-tags style="margin-left: 20px"
								placeholder="请选择">
								<div style="width: 100%; text-align: center; margin-bottom: 5px" @click="
                    () => {
                  ...
                  ">
								...
								<el-option v-for="item in adminroleList" :key="item.id" :label="item.name"
					...
					<el-table-column prop="img" label="图片" align="center" min-width="160">
						<template slot-scope="scope">
							<el-upload class="avatar-uploader" accept="image/*" :headers="headers"
								action="https://api.yhjerp.com/api/Admin/uploads" :show-file-list="false"
								:disabled="!!detailForm.id" :before-upload="beforeUpload" :on-success="
                  (res, file) => {
                    if (loadingInstance) {
			...
			<el-button type="success" @click="addFn" plain style="width: 100%">添加工序</el-button>
