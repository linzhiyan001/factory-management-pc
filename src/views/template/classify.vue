<template>
	<div class="app-container">
		...
		<div class="content">
			<div class="lgm-search">
				<el-form ref="form" :model="searchParams" label-width="80px" class="search-form">
					<el-form-item label="分类">
						<el-input v-model="searchParams.name" placeholder="请输入分类查询"></el-input>
					</el-form-item>
					<el-row>
						<el-button @click="
                    page = 1;
			...
			<div class="table">
				<el-table :data="dataList" border>
					...
					<el-table-column prop="name" label="分类值" align="center"></el-table-column>
		...
		<!-- 修改 -->
		<el-dialog title="分类" :visible.sync="dialogVisible" center width="500px">
			<el-form :model="detailForm" :rules="rules" ref="ruleForm" label-width="100px" class="detailForm">
				<el-form-item label="分类" prop="name" style="width: 100%;">
	...
	export default {
		...
		methods: {
			...
			dialogConfirm() {
				...
				clothing_categoryAdmin(this.detailForm, this.detailForm.id ? 'PATCH' : 'POST', this.detailForm.id).then((e) => {
			...
			handleDelete(index, item) {
				this.$confirm('是否确认删除选中的分类?', '提示', {
					...
					clothing_categoryAdmin({}, 'DELETE', item.id).then((e) => {
						this.seach()
