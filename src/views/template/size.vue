<template>
	<div class="app-container">
		...
		<div class="content">
			<div class="lgm-search">
				<el-form ref="form" :model="searchParams" label-width="80px" class="search-form">
					<el-form-item label="尺寸">
						<el-input v-model="searchParams.size_name" placeholder="请输入尺寸查询"></el-input>
					</el-form-item>
					<el-row>
						<el-button @click="
                    page = 1;
			...
			<div class="table">
				<el-table :data="dataList" border>
					...
					<el-table-column prop="size_name" label="尺寸值" align="center"></el-table-column>
		...
		<!-- 修改 -->
		<el-dialog title="尺寸" :visible.sync="dialogVisible" center width="500px">
			<el-form :model="detailForm" :rules="rules" ref="ruleForm" label-width="100px" class="detailForm">
				<el-form-item label="尺寸" prop="size_name" style="width: 100%;">
					<el-input v-model="detailForm.size_name"></el-input>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			dialogConfirm() {
				...
				sizeAdmin(this.detailForm, this.detailForm.id ? 'PATCH' : 'POST', this.detailForm.id).then((e) => {
			...
			handleDelete(index, item) {
				this.$confirm('是否确认删除选中的尺寸?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					confirmButtonClass: 'submitDeleteBtn',
					type: 'warning'
				}).then(() => {
					sizeAdmin({}, 'DELETE', item.id).then((e) => {
						this.seach()
