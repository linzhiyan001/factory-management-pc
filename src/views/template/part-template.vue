	...
	<div class="app-container">
		...
		<div class="content">
			<div class="lgm-search">
				<el-form ref="form" :model="searchParams" label-width="80px" class="search-form">
					<el-form-item label="部位">
						<el-input v-model="searchParams.template_name" placeholder="请输入部位查询"></el-input>
					</el-form-item>
					<el-row>
						<el-button @click="
                     page = 1;
                     part_templateAdminFn();
			...
			<div class="table">
				<el-table :data="dataList" border>
					<el-table-column prop="template_name" label="模版名称" align="center"></el-table-column>
					<el-table-column prop="part_name" label="部位" align="center"></el-table-column>
		...
		<el-dialog title="部位模版" :visible.sync="dialogVisible" center width="500px">
			<el-form :model="detailForm" :rules="rules" ref="ruleForm" label-width="100px" class="detailForm">
				<el-form-item label="模版名称" prop="template_name" style="width: 100%;">
					<el-input v-model="detailForm.template_name"></el-input>
				</el-form-item>
				
				<el-form-item label="部位" prop="part_ids" style="width: 100%;">
					<el-checkbox-group v-model="detailForm.part_ids">
	...
	export default {
		...
		methods: {
			...
			dialogConfirm() {
				...
				part_templateAdmin(this.detailForm, this.detailForm.id ? 'PATCH' : 'POST', this.detailForm.id).then((
					e) => {
			...
			handleEdit(idx, item) {
				item.part_ids=item.part_ids.map(Number);
			...
			handleDelete(index, item) {
				this.$confirm('是否确认删除选中的部位?', '提示', {
					...
					part_templateAdmin({}, 'DELETE', item.id).then((e) => {
						this.seach()
